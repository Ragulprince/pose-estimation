...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    for kid in range(num_kpts):
        ...
        if not (x_coord % 640 == 0 or y_coord % 640 == 0):
            ...
            li.append([int(x_coord),int(y_coord)])
        #print("length",len(li1))
        #li.append(li1)
    #print(len(li))
    angle1=0
    if(len(li)>=3):
        angle1=(getAngle(li[-1],li[-2],li[-3]))*-1
   
    print("angel",angle1)
    font=cv2.FONT_HERSHEY_TRIPLEX
    f=""
    if((angle1<=25 and angle1>=-10)):
        print("correct")
        f="Correct"
    else: 
        print("IC")
    ...
    #cv2.putText(im,f+str(angle1),(int(x_coord),int(y_coord)),font,1,(0,255,255),2,cv2.LINE_4)
    #print("points",li)
    ...
    for sk_id, sk in enumerate(skeleton):
        ...
        if(f=="Incorrect"):
            cv2.putText(im,f,(int(x_coord+50),int(y_coord+50)),font,1,(0,0,255),5,cv2.LINE_8)
        elif(f=="Correct"):
            cv2.putText(im,f,(int(x_coord+50),int(y_coord+50)),font,1,(0,255,255),5,cv2.LINE_4)
